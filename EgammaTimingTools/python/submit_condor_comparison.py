...
def prepare_shell(shell_path, commands, cmssw_dir, workdir_expr):
  with open(shell_path, "w") as shell:
    ...
    shell.write("run_step() {\n")
    shell.write("  local step_name=\"$1\"\n")
    shell.write("  local step_script=\"$2\"\n")
    shell.write("  local step_log=\"$WORKDIR/${step_name}.log\"\n")
    shell.write("  echo \"[run] ${step_name}\"\n")
    shell.write("  if bash \"$step_script\" >\"$step_log\" 2>&1; then\n")
    shell.write("    echo \"[ok] ${step_name}\"\n")
    shell.write("    rm -f \"$step_script\" \"$step_log\"\n")
    shell.write("  else\n")
    shell.write("    local exit_code=$?\n")
    shell.write("    echo \"[fail] ${step_name} (exit ${exit_code})\"\n")
    shell.write("    echo \"[tail] ${step_log}\"\n")
    shell.write("    tail -n 200 \"$step_log\" || true\n")
    shell.write("    exit ${exit_code}\n")
    shell.write("  fi\n")
    shell.write("}\n")
    for index, command in enumerate(commands):
      step_name = "step_{:02d}".format(index)
      step_script = "$WORKDIR/{}.sh".format(step_name)
      shell.write("cat > {} <<'__CODEX_STEP__'\n".format(step_script))
      shell.write(command)
      shell.write("\n")
      shell.write("__CODEX_STEP__\n")
      shell.write("chmod +x {}\n".format(step_script))
      shell.write("run_step {} {}\n".format(step_name, step_script))
