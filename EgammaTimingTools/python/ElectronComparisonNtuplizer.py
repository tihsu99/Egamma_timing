...
options.register("offlineSuperClusterLabel", "", VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "Offline HGCal supercluster collection")
...
offline_supercluster_label = options.offlineSuperClusterLabel
if not len(offline_supercluster_label):
  offline_supercluster_label = "particleFlowSuperClusterHGCal::%s" % (options.offlineProcess)
...
process.ntuplizer = cms.EDAnalyzer(
    ...
    electronProducer=cms.InputTag(options.electronLabel, "", options.offlineProcess),
    ...
    superClusterSrc=cms.InputTag(*offline_supercluster_label.split(":")),
