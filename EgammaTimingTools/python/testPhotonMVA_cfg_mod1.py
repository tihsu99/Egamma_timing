...
process = cms.Process("PhotonMVANtuplizer", eras.Phase2C17I13M9)
...
options.register("mvaVariablesFile",
    "RecoEgamma/PhotonIdentification/data/PhotonMVAEstimatorRun2VariablesFall17V1p1.txt", # Default value
    VarParsing.VarParsing.multiplicity.singleton, # singleton or list
    VarParsing.VarParsing.varType.string, # string, int, or float
...
options.register("photonLabel",
    "gedPhotons", # Default value
    VarParsing.VarParsing.multiplicity.singleton, # singleton or list
    VarParsing.VarParsing.varType.string, # string, int, or float
    "Electron collection name (e.g. gedPhotons, ecalDrivenGsfElectronsHGC)" # Description
)
...
photonTrackIsolation = cms.EDProducer("EgammaPhotonTkIsolationProducerNew",
...
process.photonTrackIsolationMod1 = photonTrackIsolation.clone(
    dtRef = cms.int32(1),
    dtType = cms.int32(1),
    dtMax = cms.double(3.0),
    mazVtxDist = cms.double(999.0)
)
    ...
    for idtRef, dtRef in enumerate(['PV', 'SIG']):
        for idtType, dtType in enumerate(['Dt', 'DtSigni']):
            ...
            for idz, dzMax in enumerate(l_dzMax):
                ...
                prodLabel = "photonTrkIso%s%s" %(dtMaxStr, dzMaxStr)
    
                setattr(
                    process,
                    prodLabel,
                    photonTrackIsolation.clone(
            ...
            for iMtdMva, MtdMvaMin in enumerate(l_MtdMvaMin):
                ...
                prodLabel = "photonTrkIso%s%s" %(dtMaxStr, MtdMvaMinStr)
            ...
            for iptMin, ptMin in enumerate(l_ptMin):
                ...
                prodLabel = "photonTrkIso%s%s" %(dtMaxStr, ptMinStr)
...
#    process.photonTrackIsolationMod1
