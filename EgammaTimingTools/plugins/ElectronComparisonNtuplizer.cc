...
void ElectronComparisonNtuplizer::resetOnlineVectors() {
...
void ElectronComparisonNtuplizer::fillOnlineBranches(const reco::RecoEcalCandidate& candidate,
                                                     int candidateIndex,
...
void ElectronComparisonNtuplizer::fillGenBranches(const edm::Event& iEvent,
  ...
  if (onlineMatch.first >= 0) {
    ...
    if (objectMatch.first >= 0) {
      genTree_online_trkIso_ = egammaTiming::getEgammaVar(onlineObjects[objectMatch.first], "hltEgammaEleGsfTrackIsoUnseeded");
